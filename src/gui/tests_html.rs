    ...
    fn html_supports_english_default_and_japanese_toggle() {
        ...
        assert!(INDEX_HTML.contains("'DB検出': 'DB Discovery'"));
    ...
    fn html_localizes_dynamic_render_labels() {
        ...
        assert!(INDEX_HTML.contains(r#"data-setup-page="${escapeHtml(item.page)}""#));
        assert!(INDEX_HTML.contains("$('setupChecklist').addEventListener('click'"));
        assert!(!INDEX_HTML.contains("saveSetupSettings"));
        ...
        assert!(INDEX_HTML.contains(r#"<details id="settingsDiscoverySection" class="settings-section" open>"#));
        ...
        assert!(INDEX_HTML.contains(r#"data-path-accept="db,sqlite,sqlite3""#));
        assert!(INDEX_HTML.contains(r#"data-path-kind-filter="dir""#));
        ...
        assert!(INDEX_HTML.contains("async function openPathBrowser(targetId, mode = 'path', accept = '', kindFilter = '')"));
        ...
        assert!(!INDEX_HTML.contains(">FILE<"));
        ...
        assert!(INDEX_HTML.contains("function pathBrowseAllowed(targetId)"));
        assert!(INDEX_HTML.contains("button.disabled = !allowed;"));
        assert!(INDEX_HTML.contains("message(pathBrowseDisabledMessage(targetId), true);"));
        ...
        assert!(INDEX_HTML.contains("'/api/admin/settings': 'savedSettings'"));
        assert!(INDEX_HTML.contains("permissions.allow_gui_permission_edit"));
        assert!(INDEX_HTML.contains("permissions.allow_config_edit"));
        assert!(INDEX_HTML.contains("permissionEditDisabled"));
        assert!(INDEX_HTML.contains("configEditDisabled"));
        assert!(INDEX_HTML.contains("migrationEditDisabled"));
        assert!(INDEX_HTML.contains("['lock_timeout_ms', `${settings.lock_timeout_ms} ms`]"));
        assert!(INDEX_HTML.contains("backup_keep_last: nonNegativeIntegerValue('settingsBackupKeepLast', 'Backup keep last')"));
        assert!(INDEX_HTML.contains("lock_timeout_ms: nonNegativeIntegerValue('settingsLockTimeoutMs', 'Lock timeout ms')"));
        assert!(INDEX_HTML.contains("parallel: positiveIntegerValue('settingsParallel', 'Parallel')"));
        assert!(INDEX_HTML.contains("function integerValue(id, label)"));
        assert!(INDEX_HTML.contains("if (!/^\\d+$/.test(value))"));
    ...
    fn html_uses_modal_for_migration_group_creation() {
        ...
        assert!(INDEX_HTML.contains("body: JSON.stringify({ name, dir: optionalInput('manageMigrationGroupDir'), versions: [] })"));
